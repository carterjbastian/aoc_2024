...
def part_1():
    ...
    rules = [(int(x[0]), int(x[1])) for x in [line.split("|") for line in input_arr]]
    log(rules)

    rule_designations = {}
    for before, after in rules:
        log(f"{before}, {after}")
        # If the rule designation already exists, add the new rule to it
        # Otherwise, create a new set with the new rule
        old_set = rule_designations.get(before, set())
        old_set.add(after)
        rule_designations[before] = old_set

    log(rule_designations)
    orders = real_orders if not TEST_MODE else test_orders

    good_orders = []
    for order in orders:
        is_good = True
        # This is O(n^2) but n is small so oh well
        for i in range(len(order)):
            page = order[i]
            for subsequent_page in order[i + 1 :]:
                relevant_rule_set = rule_designations.get(subsequent_page, set())
                if page not in relevant_rule_set:
                    continue
                else:
                    is_good = False
                    break
    ...
    for order in good_orders:
        ...
        middle_page = order[middle_index]
        sum += middle_page
    return sum
...
def part_2():
    ...
    for order in bad_orders:
        sub_problem = [p for p in order]
        re_ordered = []
        while len(sub_problem) > 0:
            log(f"Sub problem: {sub_problem}")
            swapped = False
            for i in range(len(sub_problem)):
                page = sub_problem[i]
                for sub_idx, subsequent_page in enumerate(sub_problem[i + 1 :]):
                    relevant_rule_set = rule_designations.get(subsequent_page, set())
                    if page not in relevant_rule_set:
                        continue
                    else:
                        # Swap sub_problem[i] and page and break from inner for loop
                        log(f"Swapping {page} and {subsequent_page}")
                        swapped = True
                        sub_problem[i], sub_problem[i + sub_idx + 1] = (
                            sub_problem[i + sub_idx + 1],
                            sub_problem[i],
                        )
                        break

            if not swapped:
                log(f"No swaps for {sub_problem[0]} - pushing to good_orders")
                re_ordered.append(sub_problem[0])
                sub_problem = sub_problem[1:]

        good_orders.append(re_ordered)
